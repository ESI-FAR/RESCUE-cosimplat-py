...
import json
import datetime
from wp3_model import your_simulation, submodel_id, total_players
...
def long_poll_with_simulation(steps):
    ...
    while current_step < len(steps):
        ...
        if current_step == 0 and not result:
            print(f"WP3 co-simulation started at step 0 by submodel_id {submodel_id}.")
            your_simulation(collected_payloads[current_step], current_step)
            current_step += 1
            continue

        if result:
            print(f"Running WP3 simulation step: {steps[current_step]}")
            for row in result:
                update_cosim_state(row, current_step)

            if check_other_players(current_step):
                print(f"All WP3 players have shared data for step {steps[current_step]}.")
                your_simulation(collected_payloads[current_step], current_step)
                current_step += 1
            else:
                print(f"Waiting for WP3 players at step {steps[current_step]}.")

        time.sleep(polling_interval)

if __name__ == '__main__':
    print("Starting WP3 long-polling with simulation...")
